...
class ManualBoxStore:
    ...
    def add_box(self, bucket_prefix: str, side: str, bbox: Dict[str, float]) -> Dict[str, object]:
        clean_bucket = (bucket_prefix or "").strip()
        if not clean_bucket:
            raise ValueError("bucket_prefix is required")
        normalized_side = (side or "").strip().lower()
...
class FaceQueueRequestHandler(SimpleHTTPRequestHandler):
    ...
    def _handle_manual_box(self) -> None:
        ...
        if "bbox" in payload:
            ...
            try:
                box = self.manual_box_store.add_box(bucket_prefix, side, payload.get("bbox") or {})
            except ValueError as exc:
                self.send_error(400, str(exc))
                return
    ...
    def _serve_next_candidate(self, parsed) -> None:
        ...
        excluded = set(self.state.labeled_ids) | set(self.state.ignored_ids) | set(skipped)
