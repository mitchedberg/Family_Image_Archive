(function () {
  ...
  const notePanels = Array.from(document.querySelectorAll('[data-note-panel]'));
      ...
      const refreshVoiceButton = document.getElementById('note-refresh-voice');
      ...
      const recorderIndicatorText = document.getElementById('recorder-indicator-text');
      let zoomAnchor = null;
      ...
      const RECORDER_POLL_INTERVAL_MS = 250;
      ...
      function setupRefreshVoiceButton() {
          if (!refreshVoiceButton) return;
          refreshVoiceButton.addEventListener('click', (e) => {
              e.stopPropagation();
              refreshVoiceData();
          });
      }
      ...
      function refreshVoiceData() {
          ...
          if (refreshVoiceButton) {
              refreshVoiceButton.disabled = true;
          }
          
          fetch(`/api/voice_data?bucket_prefix=${encodeURIComponent(bucket.bucket_prefix)}`)
              .then(resp => {
                  if (!resp.ok) throw new Error('Failed to fetch voice data');
                  return resp.json();
              })
              .then(data => {
                  if (data.status === 'ok') {
                      // Update the bucket data in place
                      bucket.voice_transcripts = data.voice_transcripts;
                      // If we are still viewing the same bucket, re-render the voice list
                      const current = state.visible[state.index];
                      if (current && current.bucket_prefix === bucket.bucket_prefix) {
                          renderVoiceTranscripts(bucket);
                          renderNoteSummary(bucket);
                      }
                      setStatus('Voice notes updated.');
                  }
              })
              .catch(err => {
                  console.error(err);
                  setStatus('Failed to refresh voice notes.', true);
              })
              .finally(() => {
                  if (refreshVoiceButton) {
                      refreshVoiceButton.disabled = false;
                  }
              });
      }
    
      function setupSourceFilters() {    if (!sourceFiltersEl) return;
  ...
  function render() {
    ...
    if (bucket && bucket.bucket_prefix) {
      localStorage.setItem('review_last_bucket_prefix', bucket.bucket_prefix);
    }
  ...
  function renderRow(bucket, idx) {
    ...
    const extras = [renderLinkedBackNotice(bucket), renderDuplicateSection(bucket)]
      .filter(Boolean)
      .join('');
    const hue = getHue(bucket.bucket_prefix);
    const bucketStyle = `background-color: hsl(${hue}, 70%, 80%); padding: 2px 6px; border-radius: 4px; color: #222;`;
    return `
      <div class="bucket-meta">
        <div class="meta-left">
          <strong>#${idx + 1} · <span style="${bucketStyle}">${bucket.bucket_prefix}</span></strong>
  ...
  function setupRecorderStatusPolling() {
    if (!recorderIndicator) return;
    fetchRecorderStatus();
    recorderPollTimer = window.setInterval(fetchRecorderStatus, RECORDER_POLL_INTERVAL_MS);
    document.addEventListener('visibilitychange', handleRecorderVisibilityChange);
  }

  function handleRecorderVisibilityChange() {
    if (!recorderIndicator) return;
    if (document.hidden) {
  ...
  function fetchRecorderStatus() {
    fetch('/api/recorder_status', { cache: 'no-store' })
      .then((response) => (response.ok ? response.json() : null))
      .then((payload) => updateRecorderIndicator(payload))
      .catch(() => updateRecorderIndicator(null));
  }

  function updateRecorderIndicator(payload) {
    if (!recorderIndicator) return;
    const active =
      payload &&
      payload.recording &&
    ...
    recorderIndicator.hidden = !active;
    if (!active) {
      if (recorderIndicatorText) {
        recorderIndicatorText.textContent = 'Recording';
      }
      return;
    }
    const bucketLabel = payload.bucketId || 'Recording';
    const modeLabel =
      Array.isArray(payload.modes) && payload.modes.length ? ` · ${payload.modes.join(' + ')}` : '';
    if (recorderIndicatorText) {
      recorderIndicatorText.textContent = `${bucketLabel}${modeLabel}`;
    }
  }
  ...
  function applyFilters(options = {}) {
    ...
    // Attempt to restore position by bucket prefix first
    const lastPrefix = localStorage.getItem('review_last_bucket_prefix');
    let targetIndex = typeof options.initialIndex === 'number' && !Number.isNaN(options.initialIndex)
        ? options.initialIndex
        : state.index;

    if (lastPrefix && options.initialIndex !== undefined) {
        const foundIndex = state.visible.findIndex(b => b.bucket_prefix === lastPrefix);
        if (foundIndex !== -1) {
