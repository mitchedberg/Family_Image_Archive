(function () {
  ...
  const auditFacesBtn = document.getElementById('audit-faces-btn');
  const auditPhotosBtn = document.getElementById('audit-photos-btn');
  const auditPhotoList = document.getElementById('audit-photo-list');
  const auditPhotoLoadMoreBtn = document.getElementById('audit-photo-load-more');
  const auditShowRejectedInput = document.getElementById('audit-show-rejected');
  const auditShowRejectedLabel = document.getElementById('audit-show-rejected-label');
  const candidatePrompt = document.getElementById('candidate-prompt');
  const candidatePromptYes = document.getElementById('candidate-prompt-yes');
  const candidatePromptNo = document.getElementById('candidate-prompt-no');
  ...
  const photoHeroAlert = document.getElementById('photo-hero-alert');
  ...
  const photoModeQueueBtn = document.getElementById('photo-mode-queue');
  const photoModeReviewBtn = document.getElementById('photo-mode-review');
  ...
  const photoRejectBucketBtn = document.getElementById('photo-reject-bucket-btn');
  const photoRejectBadge = document.getElementById('photo-reject-badge');
  ...
  const photoNukeBtn = document.getElementById('photo-nuke-btn');
  const photoDoneToggle = document.getElementById('photo-done-toggle');
  const photoDoneNextBtn = document.getElementById('photo-done-next');
  ...
  const modeClusterBtn = document.getElementById('mode-cluster-btn');
  const clusterModeSection = document.getElementById('cluster-mode');
  ...
  const clusterGridView = document.getElementById('cluster-grid-view');
  const clusterGridFooter = document.getElementById('cluster-grid-footer');
  const clusterLoadMoreBtn = document.getElementById('cluster-load-more-btn');
  const clusterRefreshBtn = document.getElementById('cluster-refresh-btn');
  const clusterMinFacesInput = document.getElementById('cluster-min-faces');
  const clusterSimilarityInput = document.getElementById('cluster-similarity');
  const clusterSimilarityValue = document.getElementById('cluster-similarity-value');
  const clusterShowLowConfBtn = document.getElementById('cluster-show-low-conf');
  const clusterReviewPanel = document.getElementById('cluster-review');
  const clusterBackBtn = document.getElementById('cluster-back-btn');
  const clusterJumpTaggerBtn = document.getElementById('cluster-jump-tagger-btn');
  const clusterReviewTitle = document.getElementById('cluster-review-title');
  const clusterReviewCount = document.getElementById('cluster-review-count');
  const clusterHeroWrapper = document.getElementById('cluster-hero-wrapper');
  const clusterHeroImg = document.getElementById('cluster-hero-img');
  const clusterHeroOverlay = document.getElementById('cluster-hero-overlay');
  const clusterFaceList = document.getElementById('cluster-face-list');
  const clusterPrevFaceBtn = document.getElementById('cluster-prev-face-btn');
  const clusterNextFaceBtn = document.getElementById('cluster-next-face-btn');
  const clusterFaceCounter = document.getElementById('cluster-face-counter');
  const clusterLabelInput = document.getElementById('cluster-label-input');
  const clusterNameBtn = document.getElementById('cluster-name-btn');
  const clusterFaceActionsPanel = document.getElementById('cluster-face-actions');
  const clusterFaceLabelSelect = document.getElementById('cluster-face-label-select');
  const clusterFaceAssignBtn = document.getElementById('cluster-face-assign-btn');
  const clusterFaceIgnoreBtn = document.getElementById('cluster-face-ignore-btn');
  ...
  let candidatePromptResolve = null;
  let candidatePromptBucket = '';
  let candidatePromptTimer = null;
  ...
  const AUDIT_PHOTO_PAGE_SIZE = 40;
  const PHOTO_PRIORITY_ORDER = { high: 0, normal: 1, low: 2 };
  const PHOTO_HERO_TIMEOUT_MS = 8000;
  const CANDIDATE_PROMPT_TIMEOUT_MS = 1500;
  ...
  const CLUSTER_MIN_FACES_STORAGE_KEY = 'face_queue.clusters.min_faces';
  const CLUSTER_SIMILARITY_STORAGE_KEY = 'face_queue.clusters.similarity';
  ...
  const state = {
    ...
    clusters: {
      ...
      hasMore: false,
      minFaces: parseInt(getStoredPreference(CLUSTER_MIN_FACES_STORAGE_KEY, '3'), 10),
      similarity: parseFloat(getStoredPreference(CLUSTER_SIMILARITY_STORAGE_KEY, '0.83')),
      metadata: {},
      view: 'grid',
      activeClusterId: '',
      focusIndex: 0,
  ...
  function init() {
    ...
    if (clusterMinFacesInput) {
      clusterMinFacesInput.value = state.clusters.minFaces;
    }
    if (clusterSimilarityInput) {
      clusterSimilarityInput.value = state.clusters.similarity.toFixed(2);
      if (clusterSimilarityValue) {
        clusterSimilarityValue.textContent = state.clusters.similarity.toFixed(2);
      }
    }
  ...
  function setupControls() {
    ...
    auditFacesBtn?.addEventListener('click', () => setAuditMode('faces'));
    auditPhotosBtn?.addEventListener('click', () => setAuditMode('photos'));
    auditPhotoLoadMoreBtn?.addEventListener('click', () => loadAuditPhotoPage());
    photoRejectBucketBtn?.addEventListener('click', () => rejectBucketForActiveLabel());
    auditShowRejectedInput?.addEventListener('change', () => {
      state.audit.showRejected = Boolean(auditShowRejectedInput.checked);
      if (state.audit.mode === 'photos') {
        refreshAuditPhotos(true);
      }
    });
    ...
    updateAuditShowRejectedState();
    candidatePromptYes?.addEventListener('click', () => handleCandidatePromptAnswer(true));
    candidatePromptNo?.addEventListener('click', () => handleCandidatePromptAnswer(false));
    ...
    modeClusterBtn?.addEventListener('click', () => setAppMode('clusters'));
    ...
    photoModeQueueBtn?.addEventListener('click', () => setPhotoMode('queue'));
    photoModeReviewBtn?.addEventListener('click', () => setPhotoMode('review'));
    ...
    photoHeroAlert?.addEventListener('click', handlePhotoHeroAlertClick);
    ...
    photoNukeBtn?.addEventListener('click', () => handlePhotoNuke());
    photoDoneToggle?.addEventListener('click', () => handlePhotoDoneToggle());
    photoDoneNextBtn?.addEventListener('click', handlePhotoDoneNext);
    if (photoZoomSlider) {
    ...
    clusterRefreshBtn?.addEventListener('click', () => refreshClusters(true));
    clusterLoadMoreBtn?.addEventListener('click', () => loadMoreClusters());
    clusterGridView?.addEventListener('click', handleClusterGridClick);
    clusterBackBtn?.addEventListener('click', () => closeClusterReview());
    clusterJumpTaggerBtn?.addEventListener('click', () => jumpToTaggerFromCluster());
    clusterNameBtn?.addEventListener('click', () => promptClusterLabel());
    clusterFaceAssignBtn?.addEventListener('click', () => handleClusterAccept());
    clusterFaceIgnoreBtn?.addEventListener('click', () => handleClusterIgnore());
    clusterPrevFaceBtn?.addEventListener('click', () => stepClusterFace(-1));
    clusterNextFaceBtn?.addEventListener('click', () => stepClusterFace(1));
    clusterFaceList?.addEventListener('click', handleClusterFaceListClick);
    clusterHeroOverlay?.addEventListener('click', handleClusterOverlayClick);
    if (clusterLabelInput) {
      clusterLabelInput.addEventListener('input', (event) => {
        state.clusters.label = (event.target.value || '').trim();
      });
    }
    clusterMinFacesInput?.addEventListener('change', (event) => {
      handleClusterMinFacesChange();
      setStoredPreference(CLUSTER_MIN_FACES_STORAGE_KEY, state.clusters.minFaces);
    });
    clusterSimilarityInput?.addEventListener('input', (event) => {
      const val = parseFloat(event.target.value);
      state.clusters.similarity = val;
      if (clusterSimilarityValue) {
        clusterSimilarityValue.textContent = val.toFixed(2);
      }
    });
    clusterSimilarityInput?.addEventListener('change', () => refreshClusters(true));
    clusterShowLowConfBtn?.addEventListener('change', () => {
      state.clusters.showLowConf = Boolean(clusterShowLowConfBtn.checked);
      updateClusterFaceList();
      renderClusterHero();
    });
  }
  ...
  function handleDecision(type) {
    ...
    fetch(endpoint, {
      ...
      .then(() => refreshAudit(true))
      .then(() => {
        if (type === 'accept') {
          return maybePromptCloseBucket();
        }
        return null;
      })
      .then(() => fetchCandidate())
  ...
  function handlePhotoNuke() {
    const bucketPrefix = state.photo.heroMeta?.bucket_prefix;
    if (!bucketPrefix || state.photo.nukeSaving) return;
    
    if (!window.confirm("Nuke Junk will hide all detections in this bucket below 0.90 confidence. Proceed?")) return;

    state.photo.nukeSaving = true;
    updatePhotoNukeBtn();
    
    return fetch('/api/photo/override', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ bucket_prefix: bucketPrefix, min_confidence: 0.90 }),
    })
      .then(checkResponse)
      .then(() => {
        setPhotoStatus('Bucket quarantined (0.90 cutoff).', 'success');
        refreshPhotoHero(true);
      })
      .catch((error) => {
        setPhotoStatus(error.message || 'Failed to quarantine bucket.', 'error');
      })
      .finally(() => {
        state.photo.nukeSaving = false;
        updatePhotoNukeBtn();
      });
  }

  function updatePhotoNukeBtn() {
    if (!photoNukeBtn) return;
    const bucketPrefix = state.photo.heroMeta?.bucket_prefix;
    photoNukeBtn.disabled = !bucketPrefix || state.photo.nukeSaving;
  }

  function maybePromptCloseBucket() {
    if (!candidatePrompt) return Promise.resolve();
    if (!state.candidate?.bucket_prefix) return Promise.resolve();
    if (state.view !== 'single') return Promise.resolve();
    return showCandidatePrompt(state.candidate.bucket_prefix);
  }

  function showCandidatePrompt(bucketPrefix) {
    candidatePromptBucket = bucketPrefix || '';
    candidatePrompt.classList.remove('is-hidden');
    if (candidatePromptTimer) {
      window.clearTimeout(candidatePromptTimer);
      candidatePromptTimer = null;
    }
    candidatePromptTimer = window.setTimeout(() => {
      handleCandidatePromptAnswer(false);
    }, CANDIDATE_PROMPT_TIMEOUT_MS);
    return new Promise((resolve) => {
      candidatePromptResolve = resolve;
    });
  }

  function hideCandidatePrompt() {
    if (!candidatePrompt) return;
    candidatePrompt.classList.add('is-hidden');
    if (candidatePromptTimer) {
      window.clearTimeout(candidatePromptTimer);
      candidatePromptTimer = null;
    }
  ...
  function isCandidatePromptVisible() {
    return Boolean(candidatePrompt && !candidatePrompt.classList.contains('is-hidden'));
  }

  function handleCandidatePromptAnswer(markDone) {
    if (!candidatePromptResolve) {
      hideCandidatePrompt();
      return;
    }
    if (candidatePromptTimer) {
    ...
    const resolve = candidatePromptResolve;
    candidatePromptResolve = null;
    const bucketPrefix = candidatePromptBucket;
    candidatePromptBucket = '';
    const finalize = () => {
      hideCandidatePrompt();
      resolve();
    };
    if (!markDone || !bucketPrefix) {
      finalize();
      return;
    }
    setPhotoDoneForBucket(bucketPrefix, true)
      .then(() => finalize())
      .catch(() => finalize());
  }
  ...
  function renderCandidate() {
    ...
    const renderActiveOverlay = () => {
      const activeId = state.candidate?.face_id || activeFace[0]?.face_id;
      renderCandidateFacesOverlay(overlayLayer, candidateImage, img, activeFace, activeId);
    };
    if (img.complete) {
      renderActiveOverlay();
    } else {
    ...
    fetch(`/api/face/${encodeURIComponent(faceId)}/context`)
      ...
      .then((context) => {
        ...
        state.candidateContextFaces = faces;
        const activeId = state.candidate?.face_id || context.face?.face_id || faceId;
        const rerender = () => renderCandidateFacesOverlay(overlayLayer, candidateImage, img, faces, activeId);
        rerender();
        ...
        const clickHandler = (event) => {
          ...
          selectCandidateFace(target.dataset.faceId);
        };
        overlayLayer.addEventListener('click', clickHandler);
        resizeHandlers.push(() => overlayLayer.removeEventListener('click', clickHandler));
        cleanupCandidateOverlay = () => {
  ...
  function selectCandidateFace(faceId) {
    if (!faceId || !state.candidateContextFaces?.length) return;
    if (state.candidate?.face_id === faceId) return;
    const match = state.candidateContextFaces.find((face) => face.face_id === faceId);
    if (!match) return;
    const next = { ...state.candidate, ...match };
    if (typeof next.similarity !== 'number' && typeof state.candidate?.similarity === 'number') {
      next.similarity = state.candidate.similarity;
    }
  ...
  function activateLabel(labelName) {
    ...
    loadLabelRejects(labelName);
    refreshAudit(true);
  }

  function loadLabelRejects(labelName) {
    if (!state.labelRejects) {
      state.labelRejects = { label: '', buckets: new Map(), requestId: 0 };
    }
    const normalized = (labelName || '').trim();
    if (!normalized) {
      state.labelRejects.label = '';
      state.labelRejects.buckets = new Map();
      updateAuditShowRejectedState();
      updatePhotoRejectUI();
      return Promise.resolve();
    }
    state.labelRejects.label = normalized;
    state.labelRejects.requestId += 1;
    const requestId = state.labelRejects.requestId;
    const params = new URLSearchParams({ label: normalized });
    return fetch(`/api/label/reject_buckets?${params.toString()}`)
      .then(checkResponse)
      .then((payload) => {
        if (state.labelRejects.requestId !== requestId || state.activeLabel !== normalized) {
          return;
        }
        const rows = Array.isArray(payload?.rejects) ? payload.rejects : [];
        const map = new Map();
        rows.forEach((row) => {
          const bucket = (row?.bucket_prefix || '').trim();
          if (bucket) {
            map.set(bucket, row);
          }
        });
        state.labelRejects.buckets = map;
      })
      .catch(() => {
        if (state.labelRejects.requestId === requestId && state.activeLabel === normalized) {
          state.labelRejects.buckets = new Map();
      ...
      .finally(() => {
        if (state.activeLabel === normalized) {
          updateAuditShowRejectedState();
          updatePhotoRejectUI();
        }
      });
  ...
  function getActiveRejectMap() {
    if (!state.labelRejects) {
      state.labelRejects = { label: '', buckets: new Map(), requestId: 0 };
    }
    if (state.labelRejects.label !== state.activeLabel) {
      return null;
    }
    return state.labelRejects.buckets;
  }

  function getLabelRejectEntry(bucketPrefix) {
    const bucket = (bucketPrefix || '').trim();
    if (!bucket) return null;
    const map = getActiveRejectMap();
    if (!map) return null;
    return map.get(bucket) || null;
  }

  function isBucketRejectedForActiveLabel(bucketPrefix) {
    return Boolean(getLabelRejectEntry(bucketPrefix));
  }

  function cacheRejectedBucket(bucketPrefix, record) {
    if (!state.labelRejects) {
      state.labelRejects = { label: state.activeLabel || '', buckets: new Map(), requestId: 0 };
    }
    if (state.labelRejects.label !== state.activeLabel) {
      state.labelRejects.label = state.activeLabel || '';
      state.labelRejects.buckets = new Map();
    }
    const entry =
      (record && typeof record === 'object'
        ? record
        : { label: state.activeLabel, bucket_prefix: bucketPrefix, updated_at_utc: new Date().toISOString() }) || {};
    state.labelRejects.buckets.set(bucketPrefix, entry);
  }

  function removeRejectedBucket(bucketPrefix) {
    const map = getActiveRejectMap();
    if (!map) return;
    map.delete(bucketPrefix);
  }

  function updateAuditShowRejectedState() {
    if (auditShowRejectedInput) {
      const hasLabel = Boolean(state.activeLabel);
      if (!hasLabel) {
        state.audit.showRejected = false;
        auditShowRejectedInput.checked = false;
      } else {
        auditShowRejectedInput.checked = Boolean(state.audit.showRejected);
      }
      auditShowRejectedInput.disabled = !hasLabel;
    }
    if (auditShowRejectedLabel) {
      const count =
        state.labelRejects && state.labelRejects.label === state.activeLabel
          ? state.labelRejects.buckets?.size || 0
          : 0;
      auditShowRejectedLabel.textContent = count > 0 ? `Show rejected (${count})` : 'Show rejected';
    }
  }

  function updatePhotoRejectUI() {
    const hasLabel = Boolean(state.activeLabel);
    const bucketPrefix = state.photo.heroMeta?.bucket_prefix || '';
    const heroActive = Boolean(!state.photo.viewingGrid && bucketPrefix);
    const rejected = hasLabel && heroActive && isBucketRejectedForActiveLabel(bucketPrefix);
    if (photoRejectBucketBtn) {
      const shouldShowButton = heroActive && hasLabel;
      photoRejectBucketBtn.classList.toggle('is-hidden', !shouldShowButton);
      if (hasLabel) {
        photoRejectBucketBtn.textContent = `Reject Photo for ${state.activeLabel}`;
      }
      photoRejectBucketBtn.disabled = !shouldShowButton || rejected || state.bucketRejectSaving;
    }
    if (photoRejectBadge) {
      if (rejected) {
        photoRejectBadge.textContent = `Rejected for ${state.activeLabel} — press U to undo`;
        photoRejectBadge.classList.remove('is-hidden');
      } else {
        photoRejectBadge.classList.add('is-hidden');
        photoRejectBadge.textContent = '';
      }
    }
  }

  function notifyBucketAction(message, level) {
    if (state.uiMode === 'photo') {
      setPhotoStatus(message, level);
    } else {
      renderMessage(message);
    }
  }

  function getBucketPrefixForReject() {
    if (state.view === 'single' && state.mode === 'verify' && state.candidate?.bucket_prefix) {
      return state.candidate.bucket_prefix;
    }
    if (!state.photo.viewingGrid && state.photo.heroMeta?.bucket_prefix) {
      return state.photo.heroMeta.bucket_prefix;
    }
    return '';
  }

  function rejectBucketForActiveLabel(bucketPrefixOverride, options = {}) {
    if (!state.activeLabel) {
      return Promise.resolve(false);
    }
    const bucketPrefix = (bucketPrefixOverride || getBucketPrefixForReject() || '').trim();
    if (!bucketPrefix || isBucketRejectedForActiveLabel(bucketPrefix) || state.bucketRejectSaving) {
      updatePhotoRejectUI();
      return Promise.resolve(false);
    }
    state.bucketRejectSaving = true;
    updatePhotoRejectUI();
    const payload = { label: state.activeLabel, bucket_prefix: bucketPrefix };
    return fetch('/api/label/reject_bucket', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      ...
      .then(checkResponse)
      .then((response) => {
        cacheRejectedBucket(bucketPrefix, response?.record || null);
        notifyBucketAction(`Rejected for ${state.activeLabel}.`, 'success');
        refreshAudit(true);
        if (options.advanceCandidate && state.view === 'single' && state.mode === 'verify') {
          fetchCandidate();
        }
        return true;
      })
      .catch((error) => {
        notifyBucketAction(error.message || 'Failed to reject photo.', 'error');
        return false;
      })
      .finally(() => {
        state.bucketRejectSaving = false;
        updatePhotoRejectUI();
        updateAuditShowRejectedState();
      });
  }

  function unrejectBucketForActiveLabel(bucketPrefixOverride) {
    if (!state.activeLabel) {
      return Promise.resolve(false);
    }
    const bucketPrefix = (bucketPrefixOverride || getBucketPrefixForReject() || '').trim();
    ...
    updatePhotoRejectUI();
    const payload = { label: state.activeLabel, bucket_prefix: bucketPrefix };
    return fetch('/api/label/unreject_bucket', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(payload),
    })
      .then(checkResponse)
      .then(() => {
        removeRejectedBucket(bucketPrefix);
        notifyBucketAction(`Rejection cleared for ${state.activeLabel}.`, 'success');
        refreshAudit(true);
        return true;
      })
      .catch((error) => {
        notifyBucketAction(error.message || 'Failed to undo rejection.', 'error');
        return false;
      })
      .finally(() => {
        state.bucketRejectSaving = false;
  ...
  function maybeHandleBucketUnreject() {
    const bucketPrefix = (getBucketPrefixForReject() || '').trim();
    if (!bucketPrefix || !isBucketRejectedForActiveLabel(bucketPrefix)) {
      return false;
    }
    unrejectBucketForActiveLabel(bucketPrefix);
    return true;
  }
  ...
  function setAuditMode(mode) {
    const next = mode === 'photos' ? 'photos' : 'faces';
    if (state.audit.mode === next) return;
    state.audit.mode = next;
    if (auditFacesBtn) {
      auditFacesBtn.classList.toggle('is-active', next === 'faces');
    }
    if (auditPhotosBtn) {
      auditPhotosBtn.classList.toggle('is-active', next === 'photos');
    }
    if (auditList) {
      auditList.classList.toggle('is-hidden', next !== 'faces');
    }
    if (auditPhotoList) {
      auditPhotoList.classList.toggle('is-hidden', next !== 'photos');
    }
    if (auditPhotoLoadMoreBtn) {
      auditPhotoLoadMoreBtn.classList.toggle('is-hidden', next !== 'photos');
    }
    refreshAudit(true);
  }
  ...
  function refreshAudit(force) {
    if (state.audit.mode === 'photos') {
      refreshAuditPhotos(force);
      return;
    }
    refreshAuditFaces(force);
  }

  function refreshAuditFaces(force) {
    if (!auditList) return;
    if (!state.activeLabel) {
    ...
    fetch(`/api/labels/detail?${params.toString()}`)
      .then(checkResponse)
      .then((payload) => {
        renderAuditFaces(payload.faces || []);
      })
      .catch((error) => {
  ...
  function renderAuditFaces(faces) {
    if (!auditList) return;
    if (!faces.length) {
  ...
  function refreshAuditPhotos(force) {
    if (!auditPhotoList) return;
    updateAuditShowRejectedState();
    if (!state.activeLabel) {
      auditPhotoList.innerHTML = '<p class="empty">Select a label to see confirmed photos.</p>';
      return;
    }
    if (!force && auditPhotoList.dataset.label === state.activeLabel) {
      return;
    }
    state.audit.photos = [];
    state.audit.photoOffset = 0;
    state.audit.photoTotal = 0;
    state.audit.photoHasMore = false;
    auditPhotoList.innerHTML = '<p class="empty">Loading…</p>';
    auditPhotoList.dataset.label = state.activeLabel;
    loadAuditPhotoPage({ reset: true });
  }

  function loadAuditPhotoPage(options = {}) {
    if (!auditPhotoList || !state.activeLabel) return;
    if (state.audit.photoLoading) return;
    const reset = Boolean(options.reset);
    const offset = reset ? 0 : state.audit.photoOffset;
    state.audit.photoLoading = true;
    if (auditPhotoLoadMoreBtn) {
      auditPhotoLoadMoreBtn.disabled = true;
      auditPhotoLoadMoreBtn.textContent = 'Loading…';
    }
    const params = new URLSearchParams({
      label: state.activeLabel,
      limit: String(AUDIT_PHOTO_PAGE_SIZE),
      offset: String(offset),
    });
    if (state.audit.showRejected) {
      params.set('include_rejected', '1');
    }
    fetch(`/api/label/photos?${params.toString()}`)
      .then(checkResponse)
      .then((payload) => {
        const photos = Array.isArray(payload?.photos) ? payload.photos : [];
        if (reset) {
          state.audit.photos = photos.slice();
        } else {
          state.audit.photos = state.audit.photos.concat(photos);
        }
        state.audit.photoTotal =
          typeof payload?.total_photos === 'number' ? payload.total_photos : state.audit.photos.length;
        state.audit.photoHasMore = Boolean(payload?.has_more);
        state.audit.photoOffset =
          typeof payload?.next_offset === 'number' ? payload.next_offset : state.audit.photos.length;
        renderAuditPhotos();
      })
      .catch((error) => {
        auditPhotoList.innerHTML = `<p class="empty">${error.message || 'Failed to load confirmed photos.'}</p>`;
      })
      .finally(() => {
        state.audit.photoLoading = false;
        if (auditPhotoLoadMoreBtn) {
          auditPhotoLoadMoreBtn.disabled = !state.audit.photoHasMore;
          auditPhotoLoadMoreBtn.textContent = state.audit.photoHasMore ? 'Load more' : 'No more';
        }
      });
  }

  function renderAuditPhotos() {
    if (!auditPhotoList) return;
    if (!state.audit.photos.length) {
      auditPhotoList.innerHTML = '<p class="empty">No confirmed photos yet.</p>';
      return;
    }
    auditPhotoList.innerHTML = '';
    const fragment = document.createDocumentFragment();
    state.audit.photos.forEach((photo, index) => {
      fragment.appendChild(buildAuditPhotoCard(photo, index));
    });
    auditPhotoList.appendChild(fragment);
  }

  function buildAuditPhotoCard(photo, index) {
    const card = document.createElement('div');
    card.className = 'audit-photo-card';
    const imageWrap = document.createElement('div');
    imageWrap.className = 'audit-photo-card__image';
    const img = document.createElement('img');
    const hasFront = photo.has_front !== false;
    const previewUrl = hasFront ? photo.front_url || photo.image_url : photo.back_url || '';
    if (previewUrl) {
      img.src = previewUrl;
    }
    ...
    card.appendChild(imageWrap);
    const meta = document.createElement('div');
    meta.className = 'audit-photo-card__meta';
    const title = document.createElement('strong');
    title.textContent = photo.bucket_prefix || 'Photo';
    meta.appendChild(title);
    const badges = document.createElement('div');
    badges.className = 'audit-photo-card__badges';
    const labeled = document.createElement('span');
    labeled.textContent = `${photo.labeled_count || 0} labeled`;
    badges.appendChild(labeled);
    const unlabeled = document.createElement('span');
    unlabeled.textContent = `${photo.unlabeled_count || 0} unlabeled`;
    badges.appendChild(unlabeled);
    if (photo.done) {
      const done = document.createElement('span');
      done.textContent = 'Done';
      badges.appendChild(done);
    }
    if (photo.rejected_for_label) {
      const rejectedBadge = document.createElement('span');
      rejectedBadge.className = 'audit-photo-card__badge audit-photo-card__badge--reject';
      rejectedBadge.textContent = `Rejected for ${state.activeLabel || 'label'} (U to undo)`;
      badges.appendChild(rejectedBadge);
      card.classList.add('is-rejected');
    }
    meta.appendChild(badges);
    card.appendChild(meta);
    const actions = document.createElement('div');
    actions.className = 'audit-photo-card__actions';
    const doneBtn = document.createElement('button');
    doneBtn.type = 'button';
    doneBtn.textContent = photo.done ? 'Mark Not Done' : 'Mark Done';
    doneBtn.addEventListener('click', () => {
      setPhotoDoneForBucket(photo.bucket_prefix, !photo.done).then((ok) => {
        if (ok) {
          refreshAuditPhotos(true);
        }
      });
    ...
    openBtn.type = 'button';
    openBtn.textContent = 'Open Tagger';
    openBtn.addEventListener('click', () => openAuditPhotoHero(index));
    actions.appendChild(doneBtn);
    actions.appendChild(openBtn);
    card.appendChild(actions);
    card.addEventListener('click', (event) => {
      if (event.target.closest('button')) return;
      openAuditPhotoHero(index);
    });
    return card;
  }

  function openAuditPhotoHero(index) {
    const safeIndex = Number.isInteger(index) ? index : -1;
    if (safeIndex < 0 || safeIndex >= state.audit.photos.length) return;
    state.photo.mode = 'review';
    state.photo.externalSource = 'audit';
    updatePhotoModeButtons();
    state.photo.items = state.audit.photos.slice();
    state.photo.totalPhotos = state.photo.items.length;
    state.photo.hasMore = false;
    state.photo.nextCursor = null;
    state.photo.cursor = 0;
    state.photo.priorityFilter = 'all';
    if (photoPriorityFilter) {
      photoPriorityFilter.value = 'all';
    }
    setAppMode('photo', { skipGrid: true });
    openPhotoHero(safeIndex);
  }
  ...
  function setPhotoMode(mode) {
    const next = mode === 'review' ? 'review' : 'queue';
    if (state.photo.mode === next) return;
    state.photo.mode = next;
    state.photo.externalSource = '';
    updatePhotoModeButtons();
    refreshPhotoGrid(true);
  }

  function updatePhotoModeButtons() {
    if (photoModeQueueBtn) {
      photoModeQueueBtn.classList.toggle('is-active', state.photo.mode === 'queue');
    }
    if (photoModeReviewBtn) {
      photoModeReviewBtn.classList.toggle('is-active', state.photo.mode === 'review');
    }
  }
  ...
  function refreshPhotoGrid(showLoading = false) {
    ...
    state.photo.gridSelectedIndex = -1;
    ...
    state.photo.transformRequested = false;
    state.photo.transformRequested = false;
  ...
  function handlePhotoGridPayload(payload, { reset, cursor }) {
    ...
    sortPhotoItems();
    syncPhotoSelection();
    if (state.photo.gridSelectedIndex < 0 && state.photo.items.length) {
      state.photo.gridSelectedIndex = 0;
    } else if (state.photo.gridSelectedIndex >= state.photo.items.length) {
      state.photo.gridSelectedIndex = state.photo.items.length - 1;
    }
    renderPhotoGrid();
    updatePhotoNav();
    if (!state.photo.items.length) {
  ...
  function buildPhotoCard(photo, index) {
    ...
    if (state.photo.viewingGrid && index === state.photo.gridSelectedIndex) {
    ...
    const unlabeledCount = photo.unlabeled_count || 0;
    unlabeledBadge.textContent =
      state.photo.mode === 'review' ? `+${unlabeledCount} unlabeled` : `${unlabeledCount} unlabeled`;
    badges.appendChild(unlabeledBadge);
    if (typeof photo.labeled_count === 'number') {
      const labeledBadge = document.createElement('span');
      labeledBadge.className = 'photo-card__badge photo-card__badge--ok';
      labeledBadge.textContent = `${photo.labeled_count} labeled`;
      badges.appendChild(labeledBadge);
    }
    ...
    if (photo.done) {
      ...
      doneBadge.className = 'photo-card__badge photo-card__badge--ok';
    ...
    if (!hasFront) {
      ...
      missingBadge.className = 'photo-card__badge photo-card__badge--alert';
      missingBadge.textContent = 'Missing front';
  ...
  function updatePhotoGridSummary() {
    ...
    const parts = [];
    const modeLabel = state.photo.mode === 'review' ? 'Review' : 'Queue';
    parts.push(`${modeLabel}: ${state.photo.items.length} of ${state.photo.totalPhotos || state.photo.items.length}`);
  ...
  function showPhotoGrid(showGrid = true) {
    ...
    if (state.photo.viewingGrid) {
      ...
      if (state.photo.gridSelectedIndex < 0 && state.photo.items.length) {
        state.photo.gridSelectedIndex =
          state.photo.selectedIndex >= 0 ? state.photo.selectedIndex : 0;
      }
  ...
  function stepPhotoSelection(delta) {
    ...
    if (nextIndex < 0) {
      setPhotoStatus('Reached start of loaded photos.', 'info');
      return;
    }
  ...
  function stepPhotoGridSelection(delta) {
    if (!state.photo.viewingGrid) return;
    if (!state.photo.items.length) return;
    const next = Math.max(
      0,
      Math.min(state.photo.items.length - 1, (state.photo.gridSelectedIndex ?? 0) + delta)
    );
    state.photo.gridSelectedIndex = next;
    renderPhotoGrid();
    const card = photoGridView?.querySelector(`.photo-card[data-index="${next}"]`);
  ...
  function openPhotoHero(index) {
    ...
    state.photo.gridSelectedIndex = safeIndex;
    state.photo.heroMeta = photo;
    state.photo.viewingBack = false;
    state.photo.emptyMessage = '';
    state.photo.heroImageError = '';
    state.photo.heroImageErrorUrl = '';
    state.photo.faces = [];
    state.photo.allFaces = [];
    state.photo.activeFaceId = '';
    state.photo.manualBoxes = [];
  ...
  function hydratePhotoHeroFromMeta(photo) {
    const frontUrl = photo?.front_url || photo?.image_url || '';
    const backUrl = photo?.back_url || '';
    const hasFront = photo?.has_front === false ? false : Boolean(frontUrl);
    const hasBack = Boolean(photo?.has_back && backUrl) || Boolean(backUrl);
    state.photo.heroFrontUrl = frontUrl;
    state.photo.heroBackUrl = backUrl;
    state.photo.heroHasBack = hasBack;
    state.photo.heroHasFront = hasFront;
    state.photo.viewingBack = !hasFront && hasBack;
    const targetUrl = state.photo.viewingBack ? state.photo.heroBackUrl : state.photo.heroFrontUrl;
    if (targetUrl) {
      renderPhotoHeroImage(targetUrl);
  ...
  function syncPhotoTransformPayload(payload) {
    if (!payload) return false;
    const payloadBucket = payload.bucket_prefix || payload.bucketPrefix || '';
    if (
      payloadBucket &&
      state.photo.heroMeta &&
      state.photo.heroMeta.bucket_prefix !== payloadBucket
    ) {
      return false;
    }
    const hasFront = payload.front && typeof payload.front.rotate !== 'undefined';
    const hasBack = payload.back && typeof payload.back.rotate !== 'undefined';
    if (!hasFront && !hasBack) {
      return false;
    }
    const frontRotate = normalizeRotation(payload.front?.rotate ?? 0);
    const backRotate = normalizeRotation(payload.back?.rotate ?? 0);
    state.photo.transforms = {
      front: { rotate: frontRotate },
      back: { rotate: backRotate },
    };
  ...
  function loadManualBoxes(bucketPrefix) {
    ...
    return fetch(`/api/photo/manual_boxes?bucket_prefix=${encodeURIComponent(bucketPrefix)}&side=front`)
      ...
      .then((payload) => {
        ...
        state.photo.manualBoxes = hydrateManualBoxes(bucketPrefix, boxes);
  ...
  function hydrateManualBoxes(bucketPrefix, boxes) {
    if (!Array.isArray(boxes)) return [];
    return boxes.map((box, index) => ({
      face_id:
        typeof box.face_index === 'number' && bucketPrefix
          ? `${bucketPrefix}:${box.face_index}`
          : `${MANUAL_FACE_PREFIX}${box.id}`,
      manual: true,
      manualBoxId: box.id,
      variant: 'manual',
      confidence: 1,
  ...
  function loadPhotoFaces(photo, navToken = null) {
    ...
    const bucketPrefix = photo.bucket_prefix;
    ...
    fetch(
      `/api/photo/${encodeURIComponent(bucketPrefix)}/faces?variant=${encodeURIComponent(PHOTO_DEFAULT_VARIANT)}`,
      ...
      .then((payload) => {
        ...
        const frontUrl =
          ...
          photo.image_url ||
          '';
        const backUrl = payload?.back_url || state.photo.heroBackUrl || photo.back_url || '';
        const hasFront = payload?.has_front === false ? false : Boolean(frontUrl);
        const hasBack = Boolean((payload?.has_back && (payload?.back_url || photo.back_url)) || backUrl);
        state.photo.heroFrontUrl = frontUrl;
        state.photo.heroBackUrl = backUrl;
        state.photo.heroHasFront = hasFront;
        state.photo.heroHasBack = hasBack;
        if (state.photo.heroMeta) {
          ...
          state.photo.heroMeta.front_url = frontUrl;
          state.photo.heroMeta.has_front = hasFront;
        }
        state.photo.viewingBack = !hasFront && hasBack;
        syncPhotoTransformPayload(payload);
        updatePhotoBackToggle();
        ...
        renderPhotoFaceList();
        const targetUrl = state.photo.viewingBack ? state.photo.heroBackUrl : state.photo.heroFrontUrl;
        return renderPhotoHeroImage(targetUrl);
      })
      .then((imageLoaded) => {
        const hasFaces = Array.isArray(state.photo.faces) && state.photo.faces.length > 0;
        if (!hasFaces && !state.photo.heroImageError) {
          const emptyMessage =
            state.photo.emptyMessage || 'No faces detected in this photo (with current filters).';
          setPhotoStatus(emptyMessage, 'info');
          return;
        }
        if (imageLoaded && !state.photo.heroImageError) {
          setPhotoStatus('');
        }
      })
      .catch((error) => {
        ...
        state.photo.heroHasFront = Boolean(state.photo.heroFrontUrl);
        state.photo.viewingBack = !state.photo.heroHasFront && state.photo.heroHasBack;
        updatePhotoBackToggle();
        updatePhotoOverlayToggle();
        ...
        renderPhotoFaceList();
        const fallbackUrl = state.photo.viewingBack ? state.photo.heroBackUrl : state.photo.heroFrontUrl;
        if (fallbackUrl) {
          renderPhotoHeroImage(fallbackUrl);
        }
      })
      .finally(() => {
        ...
        if (
          bucketPrefix &&
          !state.photo.transformLoaded &&
          !state.photo.transformRequested &&
          state.photo.heroMeta?.bucket_prefix === bucketPrefix
        ) {
          loadPhotoTransform(bucketPrefix);
        }
        releasePhotoNavLock(navToken);
      });
  }

  function clearPhotoImageError() {
    state.photo.heroImageError = '';
    state.photo.heroImageErrorUrl = '';
    renderPhotoHeroAlert();
  }

  function setPhotoImageError(message, imageUrl) {
    state.photo.heroImageLoaded = false;
    state.photo.heroImageError = message || 'Failed to load photo image.';
    state.photo.heroImageErrorUrl = imageUrl || '';
    setPhotoStatus(state.photo.heroImageError, 'error');
    renderPhotoFaceList();
    renderPhotoHeroAlert();
  }
  ...
  function renderPhotoHeroImage(imageUrl) {
    ...
    if (!photoHeroImg || !imageUrl) {
      ...
      setPhotoImageError('Photo image missing.', imageUrl);
      updatePhotoBackToggle();
      updatePhotoOverlayToggle();
      return Promise.resolve(false);
    }
    ...
    return new Promise((resolve) => {
      ...
      const finish = (ok, message) => {
        ...
        photoHeroImg.removeEventListener('load', handleLoad);
        photoHeroImg.removeEventListener('error', handleError);
        if (token !== state.photo.heroImageToken) {
          resolve(false);
          return;
        }
        if (ok) {
        ...
        setPhotoImageError(message || 'Failed to load photo image.', imageUrl);
        resolve(false);
      };
      const handleLoad = () => finish(true);
      const handleError = () => finish(false, 'Failed to load photo image.');
      const handleTimeout = () => finish(false, 'Photo load timed out.');
      photoHeroImg.addEventListener('load', handleLoad, { once: true });
      photoHeroImg.addEventListener('error', handleError, { once: true });
      timeoutId = window.setTimeout(handleTimeout, PHOTO_HERO_TIMEOUT_MS);
      photoHeroImg.src = imageUrl;
      if (typeof photoHeroImg.decode === 'function') {
        photoHeroImg.decode().catch(() => null);
      }
    });
  }
  ...
  function handlePhotoOverlayClick(event) {
    ...
    const target = event.target.closest('.candidate-face-box');
    if (!target?.dataset.faceId) return;
    selectPhotoFace(target.dataset.faceId, { focusLabelFilter: true });
  }

  function handlePhotoFaceListClick(event) {
    const imageAction = event.target.closest('button[data-photo-image-action]');
    if (imageAction?.dataset.photoImageAction) {
      event.preventDefault();
      handlePhotoImageAction(imageAction.dataset.photoImageAction);
      return;
    }
    ...
    if (row?.dataset.faceId) {
      selectPhotoFace(row.dataset.faceId, { focusLabelFilter: true });
    }
  }

  function handlePhotoHeroAlertClick(event) {
    const imageAction = event.target.closest('button[data-photo-image-action]');
    if (!imageAction?.dataset.photoImageAction) return;
    event.preventDefault();
    handlePhotoImageAction(imageAction.dataset.photoImageAction);
  }

  function selectPhotoFace(faceId, options = {}) {
    if (!faceId || faceId === state.photo.activeFaceId) return;
    state.photo.activeFaceId = faceId;
    renderPhotoFaceList();
    maybeFocusLabelFilter(Boolean(options.focusLabelFilter));
  }

  function stepPhotoFaceSelection(delta) {
    const faces = state.photo.faces || [];
    if (!faces.length) return;
    const currentIndex = faces.findIndex((face) => face.face_id === state.photo.activeFaceId);
    const safeIndex = currentIndex >= 0 ? currentIndex : 0;
    const nextIndex = (safeIndex + delta + faces.length) % faces.length;
    const nextFace = faces[nextIndex];
    if (nextFace?.face_id) {
      selectPhotoFace(nextFace.face_id, { focusLabelFilter: false });
    }
  }
  ...
  function buildPhotoEmptyBanner() {
    ...
    actions.appendChild(buildPhotoEmptyAction('Mark Done', 'done'));
    actions.appendChild(buildPhotoEmptyAction('Mark Low priority', 'priority-low'));
    actions.appendChild(buildPhotoEmptyAction('Next photo', 'next-photo'));
  ...
  function buildPhotoImageErrorBanner() {
    if (!state.photo.heroImageError) return null;
    const banner = document.createElement('div');
    banner.className = 'photo-empty-banner photo-empty-banner--error';
    const message = document.createElement('p');
    message.className = 'photo-empty-banner__text';
    message.textContent = state.photo.heroImageError;
    banner.appendChild(message);

    const actions = document.createElement('div');
    actions.className = 'photo-empty-banner__actions';
    actions.appendChild(buildPhotoImageAction('Retry', 'retry'));
    actions.appendChild(buildPhotoImageAction('Mark Done Anyway', 'done'));
    actions.appendChild(buildPhotoImageAction('Skip Photo', 'skip'));
    actions.appendChild(buildPhotoImageAction('Open image URL', 'open'));
    banner.appendChild(actions);

    const meta = document.createElement('div');
    meta.className = 'photo-empty-banner__meta';
    if (state.photo.heroMeta?.bucket_prefix) {
      const label = document.createElement('span');
      label.textContent = 'Bucket:';
      const code = document.createElement('code');
      code.textContent = state.photo.heroMeta.bucket_prefix;
    ...
    if (state.photo.heroImageErrorUrl) {
      const label = document.createElement('span');
      label.textContent = 'URL:';
      const code = document.createElement('code');
      code.textContent = state.photo.heroImageErrorUrl;
      meta.appendChild(label);
      meta.appendChild(code);
    }
  ...
  function buildPhotoImageAction(label, action) {
    const button = document.createElement('button');
    button.type = 'button';
    button.className = 'photo-empty-banner__button';
    button.dataset.photoImageAction = action;
    button.textContent = label;
    return button;
  }

  function renderPhotoHeroAlert() {
    if (!photoHeroAlert) return;
    photoHeroAlert.innerHTML = '';
    const banner = buildPhotoImageErrorBanner();
    if (banner) {
      photoHeroAlert.appendChild(banner);
      photoHeroAlert.classList.remove('is-hidden');
      return;
    }
    photoHeroAlert.classList.add('is-hidden');
  }
  ...
  function handlePhotoImageAction(action) {
    if (!action) return;
    if (action === 'retry') {
      retryPhotoImageLoad();
      return;
  ...
  function retryPhotoImageLoad() {
    const targetUrl =
      state.photo.viewingBack && state.photo.heroHasBack ? state.photo.heroBackUrl : state.photo.heroFrontUrl;
    if (!targetUrl) {
      setPhotoStatus('No image URL available to retry.', 'error');
      return;
    }
    setPhotoStatus(state.photo.viewingBack ? 'Loading back…' : 'Loading front…');
    renderPhotoHeroImage(targetUrl).then((ok) => {
      if (ok) {
        setPhotoStatus('');
      }
    });
  ...
  function openPhotoImageUrl() {
    const targetUrl =
      state.photo.viewingBack && state.photo.heroHasBack ? state.photo.heroBackUrl : state.photo.heroFrontUrl;
    if (!targetUrl) {
      setPhotoStatus('No image URL available.', 'error');
      return;
    }
  ...
  function updatePhotoDoneToggle(photo) {
    if (!photoDoneToggle) return;
    const done = Boolean(photo?.done);
    photoDoneToggle.classList.toggle('is-active', done);
    photoDoneToggle.textContent = done ? 'Done' : 'Mark Done';
    photoDoneToggle.disabled = !photo;
    if (photoDoneNextBtn) {
      photoDoneNextBtn.disabled = !photo;
    }
  }

  function togglePhotoDone() {
    if (!state.photo.heroMeta) return;
    const next = !state.photo.heroMeta.done;
    setPhotoDoneForBucket(state.photo.heroMeta.bucket_prefix, next);
  }

  function handlePhotoDoneNext() {
    if (!state.photo.heroMeta) return;
    const bucketPrefix = state.photo.heroMeta.bucket_prefix;
    const currentIndex = state.photo.selectedIndex;
    setPhotoDoneForBucket(bucketPrefix, true, { suppressRefresh: true }).then((ok) => {
      if (!ok) return;
      if (state.photo.externalSource === 'audit') {
        state.audit.photos = state.audit.photos.filter((photo) => photo.bucket_prefix !== bucketPrefix);
      }
      state.photo.items = state.photo.items.filter((photo) => photo.bucket_prefix !== bucketPrefix);
      state.photo.totalPhotos = state.photo.items.length;
      if (!state.photo.items.length) {
        showPhotoGrid(true);
        return;
      }
      const nextIndex = Math.min(currentIndex, state.photo.items.length - 1);
      openPhotoHero(nextIndex);
    });
  }

  function setPhotoDoneForBucket(bucketPrefix, done, options = {}) {
    if (!bucketPrefix) return Promise.resolve(false);
    const suppressRefresh = Boolean(options.suppressRefresh);
    return fetch('/api/photo/done', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ bucket_prefix: bucketPrefix, done: Boolean(done) }),
    })
      .then(checkResponse)
      .then((payload) => {
        const record = payload?.record || {};
        const isDone = Boolean(record?.done);
        const photo = state.photo.items.find((entry) => entry.bucket_prefix === bucketPrefix);
        if (photo) {
          photo.done = isDone;
          photo.done_at = record.done_at || photo.done_at;
        }
        if (state.photo.heroMeta && state.photo.heroMeta.bucket_prefix === bucketPrefix) {
          state.photo.heroMeta.done = isDone;
          state.photo.heroMeta.done_at = record.done_at || state.photo.heroMeta.done_at;
          updatePhotoDoneToggle(state.photo.heroMeta);
        }
        if (!done && state.photo.mode === 'review') {
          setPhotoStatus('Marked not done.', 'info');
        } else if (done) {
          setPhotoStatus('Marked done.', 'success');
        }
        if (!suppressRefresh) {
          if (state.photo.mode === 'review' && isDone) {
            if (state.photo.externalSource === 'audit') {
              refreshAuditPhotos(true);
            } else {
              refreshPhotoGrid(true);
            }
          } else if (state.photo.mode === 'queue' && isDone) {
            refreshPhotoGrid(true);
          } else {
            renderPhotoGrid();
          }
      ...
      .catch((error) => {
        setPhotoStatus(error.message || 'Failed to update done status.', 'error');
        return false;
      });
  }
  ...
  function cyclePhotoPriority() {
    if (!state.photo.heroMeta) return;
    const current = state.photo.heroMeta.priority || 'normal';
    const order = ['high', 'normal', 'low'];
    const idx = order.indexOf(current);
    const next = order[(idx + 1) % order.length];
    setPhotoPriority(next);
  }
  ...
  function applyPhotoFaceFilters() {
    ...
    const seen = new Set();
    faces.forEach((face) => {
      if (!face?.face_id || seen.has(face.face_id)) return;
  ...
  function forceReleasePhotoNavLock() {
    releasePhotoNavLock(state.photo.navToken);
  }
  ...
  function handlePhotoBackToggle() {
    ...
    const requestedBack = state.photo.viewingBack;
    state.photo.drawMode = false;
    state.photo.drawing = false;
    clearPhotoDrawBox();
    resetPhotoZoomState({ unlock: true });
    ...
    if (!targetUrl) {
      state.photo.viewingBack = false;
      updatePhotoBackToggle();
      setPhotoStatus(requestedBack ? 'Back image not available.' : 'Front image not available.', 'error');
      return;
    }
    setPhotoStatus(state.photo.viewingBack ? 'Loading back…' : 'Loading front…');
    renderPhotoHeroImage(targetUrl).then((ok) => {
  ...
  function updatePhotoBackToggle() {
    ...
    const hasFront = Boolean(state.photo.heroHasFront);
    ...
    photoBackToggle.disabled = !hasFront;
    if (!hasFront) {
      photoBackToggle.textContent = 'Back only';
      photoBackToggle.setAttribute('aria-pressed', 'true');
    } else {
  ...
  function handlePhotoModeHotkeys(event, key) {
    ...
    if (state.photo.viewingGrid) {
      ...
      if (key === 'enter') {
        event.preventDefault();
        if (state.photo.gridSelectedIndex >= 0) {
          openPhotoHero(state.photo.gridSelectedIndex);
        }
        return true;
      }
      if (key === 'd') {
        event.preventDefault();
        const photo = state.photo.items[state.photo.gridSelectedIndex];
        if (photo?.bucket_prefix) {
          setPhotoDoneForBucket(photo.bucket_prefix, !photo.done);
        }
        return true;
      }
      return false;
    ...
    if (key === 'tab') {
      event.preventDefault();
      stepPhotoFaceSelection(event.shiftKey ? -1 : 1);
      return true;
    }
    if (key === 'enter') {
      if (state.photo.activeFaceId) {
        event.preventDefault();
        assignFaceToActiveLabel(state.photo.activeFaceId);
        return true;
      }
    }
    if (key === 'd') {
    ...
    if (key === 'b') {
      event.preventDefault();
      handlePhotoBackToggle();
      return true;
    }
    if (key === 'l') {
      event.preventDefault();
      setPhotoPriority('low');
      return true;
    }
    if (key === 'n') {
      event.preventDefault();
  ...
  function getRenderedImageRect(wrapper, img) {
    ...
    // If wrapper is collapsed, try to use its calculated offsetHeight or parent
    if (containerWidth <= 1) containerWidth = wrapper.offsetWidth || 600;
    if (containerHeight <= 1) containerHeight = wrapper.offsetHeight || 420;
  ...
  function handleHotkeys(event) {
    ...
    if (isCandidatePromptVisible()) {
      if (key === 'y') {
        event.preventDefault();
        handleCandidatePromptAnswer(true);
        return;
      }
      if (key === 'n' || key === 'escape') {
        event.preventDefault();
    ...
    if (state.uiMode === 'clusters') {
      if (handleClusterHotkeys(event, key)) {
        return;
      }
    }
    if (state.uiMode === 'photo') {
    ...
    switch (key) {
      ...
      case 'f':
        if (event.shiftKey) {
          event.preventDefault();
          rejectBucketForActiveLabel(null, { advanceCandidate: state.mode === 'verify' && state.view === 'single' });
        } else if (state.mode === 'verify' && state.view === 'single') {
          handleDecision('reject');
        }
        break;
      case 's':
  ...
  function handleClusterHotkeys(event, key) {
    if (state.uiMode !== 'clusters') return false;
    if (state.clusters.view === 'grid') {
      switch (key) {
        case 'enter':
          event.preventDefault();
          openFocusedCluster();
  ...
  function setClusterStatus(message = '', level = 'info') {
    if (!clusterStatusEl) return;
    clusterStatusEl.textContent = message || '';
    clusterStatusEl.classList.toggle('is-error', Boolean(message) && level === 'error');
    clusterStatusEl.classList.toggle('is-success', Boolean(message) && level === 'success');
  }

  function ensureClustersLoaded() {
    if (state.clusters.loading) {
      return;
    }
    if (!state.clusters.items.length && !state.clusters.error) {
      fetchClusters({ reset: true });
    } else {
      renderClusterGrid();
    }
  }

  function refreshClusters(force = true) {
    if (!force && state.clusters.loading) {
      return;
    }
    fetchClusters({ reset: true });
  }
  ...
  function loadMoreClusters() {
    if (state.clusters.loading || !state.clusters.hasMore) {
      return;
    }
    fetchClusters({ reset: false });
  }

  function fetchClusters(options = {}) {
    if (state.clusters.loading) {
      return;
    }
    const reset = Boolean(options.reset);
    ...
    if (reset) {
      ...
      state.clusters.focusIndex = 0;
      state.clusters.activeClusterId = '';
      renderClusterGrid();
    }
    setClusterStatus('Loading clusters…');
    const offset = reset ? 0 : state.clusters.items.length;
    const params = new URLSearchParams({
      offset: String(offset),
      limit: String(state.clusters.limit),
      min_faces: String(state.clusters.minFaces),
      similarity: String(state.clusters.similarity),
    });
    fetch(`/api/clusters?${params.toString()}`)
      .then(checkResponse)
      .then((payload) => {
        const rows = Array.isArray(payload.clusters) ? payload.clusters : [];
        state.clusters.hasMore = Boolean(payload.has_more);
        state.clusters.metadata = payload.metadata || {};
        if (reset) {
          state.clusters.items = rows;
        } else {
          state.clusters.items = state.clusters.items.concat(rows);
        }
        if (state.clusters.items.length) {
          if (state.clusters.focusIndex < 0 || state.clusters.focusIndex >= state.clusters.items.length) {
            state.clusters.focusIndex = 0;
          }
          state.clusters.activeClusterId = state.clusters.items[state.clusters.focusIndex]?.cluster_id || '';
        } else {
          state.clusters.focusIndex = -1;
          state.clusters.activeClusterId = '';
        }
      ...
      .catch((error) => {
        state.clusters.error = error.message || 'Failed to load clusters.';
        setClusterStatus(state.clusters.error, 'error');
        renderClusterGrid();
      })
      .finally(() => {
        state.clusters.loading = false;
        updateClusterLoadMoreState();
      });
  }

  function updateClusterLoadMoreState() {
    if (!clusterLoadMoreBtn) return;
    clusterLoadMoreBtn.disabled = state.clusters.loading || !state.clusters.hasMore;
    if (clusterGridFooter) {
      clusterGridFooter.classList.toggle('is-hidden', !state.clusters.hasMore);
    }
  }

  function renderClusterGrid() {
    if (!clusterGridView) return;
    clusterGridView.innerHTML = '';
    if (!state.clusters.items.length) {
      const message = state.clusters.loading
        ? 'Loading clusters…'
        : state.clusters.error || 'No clusters match the current filters.';
      const empty = document.createElement('p');
      empty.className = 'empty';
      empty.textContent = message;
      clusterGridView.appendChild(empty);
    } else {
      const fragment = document.createDocumentFragment();
      state.clusters.items.forEach((entry, index) => {
        fragment.appendChild(buildClusterCard(entry, index));
      });
      clusterGridView.appendChild(fragment);
    }
    if (clusterSummaryEl) {
      const total = state.clusters.metadata?.total || state.clusters.items.length;
      clusterSummaryEl.textContent = `${state.clusters.items.length} shown of ${total} · min faces ≥ ${state.clusters.minFaces}`;
    }
    renderClusterView();
    updateClusterLoadMoreState();
  }

  function buildClusterCard(entry, index) {
    const card = document.createElement('div');
    card.className = 'cluster-card';
    if (index === state.clusters.focusIndex) {
      card.classList.add('is-selected');
    }
    card.dataset.index = index;
    card.dataset.clusterId = entry.cluster_id || '';
    const image = document.createElement('div');
    image.className = 'cluster-card__image';
    const img = document.createElement('img');
    const imgUrl = entry.representative?.image_url || entry.representative?.image || '';
    img.src = imgUrl ? `${imgUrl}${imgUrl.includes('?') ? '&' : '?'}v=${state.generatedAt || Date.now()}` : '';
    img.alt = entry.cluster_id ? `Cluster ${entry.cluster_id}` : 'Cluster preview';
    img.onerror = () => {
      console.warn('Failed to load cluster image:', imgUrl);
    ...
    const title = document.createElement('strong');
    title.textContent = entry.representative?.bucket_prefix || entry.cluster_id || 'Cluster';
    const counts = document.createElement('span');
    counts.className = 'cluster-card__counts';
    counts.textContent = `${entry.face_count || 0} faces · ${entry.bucket_count || 0} photos`;
    meta.appendChild(title);
    ...
    const openBtn = document.createElement('button');
    openBtn.type = 'button';
    openBtn.textContent = 'Open (Enter)';
    openBtn.dataset.action = 'open';
    const nameBtn = document.createElement('button');
    nameBtn.type = 'button';
    nameBtn.textContent = 'Name (N)';
    nameBtn.dataset.action = 'name';
    actions.appendChild(openBtn);
  ...
  function renderClusterView() {
    const isReview = state.clusters.view === 'review';
    clusterReviewPanel?.classList.toggle('is-hidden', !isReview);
    clusterGridView?.classList.toggle('is-hidden', isReview);
    if (clusterGridFooter) {
      clusterGridFooter.classList.toggle('is-hidden', isReview || !state.clusters.items.length);
    }
  }

  function showClusterGrid() {
    state.clusters.view = 'grid';
    renderClusterView();
  }

  function selectClusterByIndex(index, options = {}) {
    if (!state.clusters.items.length) return;
    const clamped = Math.max(0, Math.min(index, state.clusters.items.length - 1));
    state.clusters.focusIndex = clamped;
    state.clusters.activeClusterId = state.clusters.items[clamped]?.cluster_id || '';
    renderClusterGrid();
    if (options.open && state.clusters.activeClusterId) {
      openClusterReview(state.clusters.activeClusterId);
    }
  }

  function stepClusterFocus(delta) {
    if (!state.clusters.items.length) return;
    const next = state.clusters.focusIndex + delta;
    selectClusterByIndex(Math.max(0, Math.min(next, state.clusters.items.length - 1)));
  }

  function openFocusedCluster() {
    const cluster = state.clusters.items[state.clusters.focusIndex];
    if (!cluster?.cluster_id) return;
    openClusterReview(cluster.cluster_id);
  }

  function openClusterReview(clusterId) {
    if (!clusterId) return;
    state.clusters.view = 'review';
    state.clusters.detailLoading = true;
    state.clusters.detailError = '';
    state.clusters.activeClusterId = clusterId;
    state.clusters.label = '';
    if (clusterLabelInput) clusterLabelInput.value = '';
    state.clusters.bucketFaces = {};
    renderClusterView();
    renderClusterReview();
    fetch(`/api/cluster/${clusterId}`)
      ...
      .then((payload) => {
        ...
        state.clusters.faces = Array.isArray(payload.faces) ? payload.faces : [];
        state.clusters.activeIndex = 0;
        state.clusters.activeClusterMeta = payload;
        if (!state.clusters.label && clusterLabelInput) {
          clusterLabelInput.value = '';
        }
        
        if (clusterLabelInput) {
          if (state.clusters.label) {
            clusterLabelInput.value = state.clusters.label;
          }
        }
        
        if (clusterFaceActionsPanel && clusterFaceLabelSelect) {
          clusterFaceActionsPanel.classList.remove('is-hidden');
          clusterFaceLabelSelect.innerHTML = '';
          state.labels.forEach((label) => {
            const option = document.createElement('option');
            option.value = label.name;
            option.textContent = `${label.name} (${label.count})`;
            clusterFaceLabelSelect.appendChild(option);
          });
          if (state.activeLabel) {
            clusterFaceLabelSelect.value = state.activeLabel;
          }
        }
      ...
      .catch((error) => {
        console.warn('Cluster load failed:', error);
        state.clusters.detailLoading = false;
        if (error.message.includes('404') || error.message.includes('Unknown cluster')) {
          setClusterStatus('Cluster expired; refreshing...', 'info');
          closeClusterReview();
          refreshClusters(true);
        } else {
          state.clusters.detailError = error.message || 'Failed to load cluster details.';
          renderClusterReview();
        }
      });
  }
  ...
  function jumpToTaggerFromCluster() {
    const face = getActiveClusterFace();
    if (!face?.bucket_prefix) return;
    
    const bucket = face.bucket_prefix;
    setAppMode('photo');
    setPhotoMode('queue'); // Switch to queue mode to allow tagging
    
    // Find the photo in the grid items if it exists, otherwise we will have to wait for refresh
    const existing = state.photo.items.findIndex(item => item.bucket_prefix === bucket);
    if (existing >= 0) {
      openPhotoHero(existing);
    } else {
      // Force refresh and jump to this bucket once loaded
      state.photo.externalSource = ''; 
      refreshPhotoGrid(true).then(() => {
        const idx = state.photo.items.findIndex(item => item.bucket_prefix === bucket);
        if (idx >= 0) openPhotoHero(idx);
      });
    }
  }

  function handleClusterGridClick(event) {
    const card = event.target.closest('.cluster-card');
    if (!card) return;
    const index = Number(card.dataset.index || '0');
    const action = event.target.dataset.action || '';
    selectClusterByIndex(index);
    if (action === 'open') {
      openClusterReview(card.dataset.clusterId || '');
      return;
    }
    if (action === 'name') {
      promptClusterLabel(card.dataset.clusterId || '');
      return;
    }
    openClusterReview(card.dataset.clusterId || '');
  }

  function promptClusterLabel(clusterId) {
    const targetId = clusterId || state.clusters.activeClusterId;
    if (!targetId) {
      setClusterStatus('Select a cluster first.', 'error');
      return;
    }
    let label = state.clusters.label || (clusterLabelInput?.value || '').trim();
    if (!label) {
      label = window.prompt('Enter label for this cluster', '') || '';
    }
    label = label.trim();
    if (!label) {
      return;
    }
    submitClusterLabel(targetId, label);
  }

  function submitClusterLabel(clusterId, label) {
    if (!clusterId || !label) return;
    state.clusters.naming = true;
    setClusterStatus(`Naming cluster ${clusterId}…`);
    fetch(`/api/cluster/${clusterId}/label`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      ...
      .then(checkResponse)
      .then(() => {
        state.clusters.label = label;
        if (clusterLabelInput) {
          clusterLabelInput.value = label;
        }
        setClusterStatus(`Cluster tagged as ${label}`, 'success');
        refreshClusters(true);
      })
      .catch((error) => {
        setClusterStatus(error.message || 'Failed to name cluster.', 'error');
      })
      .finally(() => {
        state.clusters.naming = false;
      });
  }

  function renderClusterReview() {
    if (!clusterReviewPanel) return;
    renderClusterView();
    if (state.clusters.view !== 'review') {
      return;
    }
    const meta = state.clusters.activeClusterMeta;
    if (clusterReviewCount) {
      const facesCount = meta?.face_count ?? state.clusters.faces.length;
      const bucketCount = meta?.bucket_count ?? (meta?.bucket_prefixes?.length || 0);
      clusterReviewCount.textContent = `${facesCount} faces · ${bucketCount} photos`;
    }
    if (clusterReviewTitle) {
      const label = state.clusters.label || meta?.representative?.label || 'Unlabeled Cluster';
      clusterReviewTitle.textContent = label;
    }
    if (clusterLabelInput) {
      if (state.clusters.label) {
        clusterLabelInput.value = state.clusters.label;
  ...
  function updateClusterFaceList() {
    if (!clusterFaceList) return;
    clusterFaceList.innerHTML = '';
    if (state.clusters.detailLoading) {
      const loading = document.createElement('li');
      loading.className = 'empty';
      loading.textContent = 'Loading faces…';
      clusterFaceList.appendChild(loading);
      return;
    }
    if (state.clusters.detailError) {
      const error = document.createElement('li');
      error.className = 'empty';
      error.textContent = state.clusters.detailError;
      clusterFaceList.appendChild(error);
      return;
    }
    if (!state.clusters.faces.length) {
      const empty = document.createElement('li');
      empty.className = 'empty';
      empty.textContent = 'No faces in this cluster.';
      clusterFaceList.appendChild(empty);
      return;
    }
    state.clusters.faces.forEach((face, index) => {
      // PR-2: Hide junk detections by default
      const isJunk = (face.confidence || 0) < 0.75 || getFaceArea(face) < 0.003;
      if (isJunk && !state.clusters.showLowConf) return;

      const item = document.createElement('li');
      item.dataset.index = index;
      if (index === state.clusters.activeIndex) {
        item.classList.add('is-active');
      }
      const name = document.createElement('strong');
      name.textContent = face.bucket_prefix || face.face_id;
      if (isJunk) {
        name.style.opacity = '0.5';
      }
      const status = document.createElement('span');
      if (face?.state?.label) {
        status.textContent = `Tagged: ${face.state.label}`;
      } else if (face?.state?.ignored) {
        status.textContent = face.state.ignore_reason ? `Ignored (${face.state.ignore_reason})` : 'Ignored';
      } else {
        status.textContent = (face.confidence ? `${face.confidence.toFixed(2)}` : '') || '';
      }
      item.appendChild(name);
      item.appendChild(status);
      clusterFaceList.appendChild(item);
    });
    if (clusterFaceCounter) {
      clusterFaceCounter.textContent = `${state.clusters.activeIndex + 1} / ${state.clusters.faces.length}`;
    }
  }

  function handleClusterFaceListClick(event) {
    const target = event.target.closest('li');
    if (!target?.dataset?.index) return;
    const index = Number(target.dataset.index);
    if (Number.isNaN(index)) return;
    setClusterActiveFace(index);
  }

  function handleClusterOverlayClick(event) {
    if (!state.clusters.faces.length || !clusterHeroImg) return;
    
    const rect = clusterHeroImg.getBoundingClientRect();
    if (rect.width === 0 || rect.height === 0) return;

    // Map click coordinates to 0.0-1.0 image space
    const clickX = (event.clientX - rect.left) / rect.width;
    const clickY = (event.clientY - rect.top) / rect.height;

    // Find all faces containing the click
    const matches = state.clusters.faces.filter((face) => {
    ...
    // Sort by smallest area (primary) and highest confidence (tie-breaker)
    matches.sort((a, b) => {
      const areaA = getFaceArea(a);
      const areaB = getFaceArea(b);
      if (Math.abs(areaA - areaB) > 0.00001) {
        return areaA - areaB; // Smallest first
      }
      return (b.confidence || 0) - (a.confidence || 0); // Highest confidence first
    });

    const bestMatchId = matches[0].face_id;
    const index = state.clusters.faces.findIndex((f) => f.face_id === bestMatchId);
    if (index >= 0) {
      setClusterActiveFace(index);
    }
  }

  function setClusterActiveFace(index) {
    if (!state.clusters.faces.length) return;
    const clamped = Math.max(0, Math.min(index, state.clusters.faces.length - 1));
    state.clusters.activeIndex = clamped;
    renderClusterHero();
    updateClusterFaceList();
  }

  function getActiveClusterFace() {
    if (!state.clusters.faces.length) return null;
    return state.clusters.faces[state.clusters.activeIndex] || null;
  }

  function stepClusterFace(delta) {
    if (!state.clusters.faces.length) return;
    let next = state.clusters.activeIndex + delta;
    if (next < 0) next = state.clusters.faces.length - 1;
    if (next >= state.clusters.faces.length) next = 0;
  ...
  function renderClusterHero() {
    ...
    if (!face) {
      ...
      clusterHeroImg.removeAttribute('src');
      if (clusterFaceCounter) clusterFaceCounter.textContent = '0 / 0';
      return;
    }
    if (clusterFaceCounter) {
      clusterFaceCounter.textContent = `${state.clusters.activeIndex + 1} / ${state.clusters.faces.length}`;
    ...
    clusterHeroOverlay.innerHTML = '';
    const token = ++state.clusters.heroToken;
    const heroUrl = face.image || face.image_url || face.imageUrl || '';
    const uniqueUrl = heroUrl ? `${heroUrl}${heroUrl.includes('?') ? '&' : '?'}v=${state.generatedAt || Date.now()}` : '';
    
    clusterHeroImg.src = uniqueUrl;
    
    // Modern way to ensure image data is ready for layout/drawing
    clusterHeroImg.decode().then(() => {
      if (state.clusters.heroToken !== token) return;
      // Double check dimensions before drawing
      if (clusterHeroImg.naturalWidth === 0) {
    ...
    clusterHeroImg.onerror = () => {
      if (state.clusters.heroToken !== token) return;
      console.error('Failed to load cluster hero image:', uniqueUrl);
      clusterHeroOverlay.innerHTML = '';
    };
  }
  ...
  function drawClusterOverlay(face, token) {
    ...
    fetch(`/api/photo/${bucket}/faces?${params.toString()}`)
      .then(checkResponse)
      .then((payload) => {
  ...
  function handleClusterMinFacesChange() {
    if (!clusterMinFacesInput) return;
    const value = parseInt(clusterMinFacesInput.value, 10);
    if (!Number.isFinite(value) || value <= 0) {
      clusterMinFacesInput.value = String(state.clusters.minFaces);
      return;
    }
    state.clusters.minFaces = value;
    refreshClusters(true);
  }

  function validateClusterLabel() {
    const label = (state.clusters.label || clusterLabelInput?.value || '').trim();
    if (!label) {
      setClusterStatus('Set a label (press N) before tagging.', 'error');
      clusterLabelInput?.focus();
      return '';
    }
    return label;
  }

  function handleClusterAccept() {
    const face = getActiveClusterFace();
    if (!face) return;
    let label = clusterFaceLabelSelect?.value;
    if (!label) {
      label = validateClusterLabel();
    }
    if (!label) return;
    fetch('/api/queue/accept', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ face_id: face.face_id, label }),
    })
      .then(checkResponse)
      .then(() => {
        face.state = face.state || {};
        face.state.label = label;
        face.state.ignored = false;
        setClusterStatus(`Accepted ${face.face_id} as ${label}`, 'success');
        updateClusterFaceList();
        renderClusterHero();
      })
      .catch((error) => setClusterStatus(error.message || 'Failed to accept face.', 'error'));
  }

  function handleClusterReject() {
    const face = getActiveClusterFace();
    const label = validateClusterLabel();
    ...
    fetch('/api/queue/reject', {
      ...
      .then(() => {
        face.state = face.state || {};
        face.state.label = null;
        setClusterStatus(`Rejected ${face.face_id} for ${label}`, 'success');
        updateClusterFaceList();
        stepClusterFace(1);
      })
      .catch((error) => setClusterStatus(error.message || 'Failed to reject face.', 'error'));
  }

  function handleClusterIgnore() {
    const face = getActiveClusterFace();
    if (!face) return;
    fetch('/api/queue/ignore', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ face_id: face.face_id, reason: 'cluster', note: 'Cluster review' }),
    })
      .then(checkResponse)
      .then(() => {
        face.state = face.state || {};
        face.state.ignored = true;
        face.state.ignore_reason = 'cluster';
        setClusterStatus(`Ignored ${face.face_id}`, 'success');
        updateClusterFaceList();
        stepClusterFace(1);
      })
      .catch((error) => setClusterStatus(error.message || 'Failed to ignore face.', 'error'));
  }

  function handleClusterDone() {
    const face = getActiveClusterFace();
    if (!face?.bucket_prefix) return;
    fetch('/api/photo/done', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ bucket_prefix: face.bucket_prefix, done: true, done_by: 'cluster_review' }),
    })
      .then(checkResponse)
      .then(() => {
        setClusterStatus(`Marked ${face.bucket_prefix} done.`, 'success');
      })
      .catch((error) => setClusterStatus(error.message || 'Failed to mark done.', 'error'));
  }
  ...
  function setAppMode(mode, options = {}) {
    const normalized = mode === 'photo' || mode === 'clusters' ? mode : 'person';
    state.uiMode = normalized;
    const isPhoto = normalized === 'photo';
    const isCluster = normalized === 'clusters';
    const skipGrid = Boolean(options.skipGrid);
    modePersonBtn?.classList.toggle('is-active', normalized === 'person');
    modePhotoBtn?.classList.toggle('is-active', isPhoto);
    modeClusterBtn?.classList.toggle('is-active', isCluster);
    personModeSection?.classList.toggle('is-hidden', normalized !== 'person');
    photoModeSection?.classList.toggle('is-hidden', !isPhoto);
    clusterModeSection?.classList.toggle('is-hidden', !isCluster);
    if (isPhoto) {
      if (!skipGrid) {
