(function () {
  ...
  function setupControls() {
    ...
    photoDoneToggle?.addEventListener('click', togglePhotoDone);
    ...
    clusterMinFacesInput?.addEventListener('change', handleClusterMinFacesChange);
  }
  ...
  function buildClusterCard(entry, index) {
    ...
    const img = document.createElement('img');
    img.src = entry.representative?.image_url || entry.representative?.image || '';
  ...
  function handleClusterOverlayClick(event) {
    const target = event.target.closest('.candidate-face-box');
    if (!target?.dataset?.faceId) return;
    const index = state.clusters.faces.findIndex((face) => face.face_id === target.dataset.faceId);
    if (index >= 0) {
      setClusterActiveFace(index);
    }
  }
  ...
  function renderClusterHero() {
    ...
    clusterHeroImg.src = face.image || face.image_url || face.imageUrl || '';
  }

  function drawClusterOverlay(face, token) {
    const bucket = face.bucket_prefix;
    if (!bucket) {
      renderCandidateFacesOverlay(clusterHeroOverlay, clusterHeroWrapper, clusterHeroImg, [face], face.face_id);
      return;
    }
    const cache = state.clusters.bucketFaces[bucket];
    if (cache) {
      renderCandidateFacesOverlay(clusterHeroOverlay, clusterHeroWrapper, clusterHeroImg, cache, face.face_id);
      return;
    }
    fetch(`/api/photo/${bucket}/faces`)
      .then(checkResponse)
      .then((payload) => {
        ...
        renderCandidateFacesOverlay(
          ...
          clusterHeroWrapper,
          clusterHeroImg,
          state.clusters.bucketFaces[bucket],
          face.face_id
        );
      })
      .catch(() => {
