(function () {
  ...
  function loadPhotoFaces(photo, navToken = null) {
    ...
    fetch(
      ...
      .then(() => {
        if (state.photo.faces.length && state.photo.heroImageLoaded) {
          setPhotoStatus('');
        }
      })
      .catch((error) => {
  ...
  function renderPhotoFaceList(initial = false) {
    ...
    const errorBanner = buildPhotoImageErrorBanner();
    if (!faces.length) {
      if (errorBanner) {
        photoFaceList.appendChild(errorBanner);
      }
      const empty = document.createElement('p');
      empty.className = 'empty';
      if (initial) {
    ...
    const fragment = document.createDocumentFragment();
    if (errorBanner) {
      fragment.appendChild(errorBanner);
    }
    faces.forEach((face) => {
