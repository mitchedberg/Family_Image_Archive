...
from archive_lib.bucket_overrides import BucketOverrideStore
from archive_lib.label_bucket_rejects import LabelBucketRejectStore
from archive_lib.label_utils import normalize_label
from archive_lib import scan_audit
from archive_lib.stores.face_people_store import FacePeopleStore
from archive_lib.stores.photo_priority_store import PhotoPriorityStore
from archive_lib.stores.photo_status_store import PhotoStatusStore
from archive_lib.stores.manual_box_store import ManualBoxStore
...
def _setup_runtime(
    *,
    ...
    label_bucket_reject_store = LabelBucketRejectStore(cfg.config_dir / "label_bucket_rejects.csv")
    bucket_override_store = BucketOverrideStore(cfg.config_dir / "bucket_overrides.json")
    state = QueueState(
        matcher,
        tag_store,
        vote_store,
        ...
        bucket_reject_store=label_bucket_reject_store,
        bucket_override_store=bucket_override_store,
    )
    return (
        cfg,
        matcher,
        ...
        manual_box_store,
        label_bucket_reject_store,
        bucket_override_store,
    )
...
def _build_and_store_clusters(
    *,
    ...
    builder = FaceClusterBuilder(
        ...
        max_bucket_size=max_bucket_size,
        logger=logger,
        min_confidence=0.75, # PR-2: stricter clustering default
        min_area=0.003,      # PR-2: filter tiny faces
    )
...
def _cluster_candidate_face_ids(
    ...
    for record in matcher.records:
        if record.face_id in excluded_face_ids:
            continue
        # Filter tiny patches that cause false positives
        area = record.bbox[2] * record.bbox[3]
        if area < MIN_FACE_AREA:
            continue
        candidates.append(record.face_id)
    candidates.sort()
    return candidates
...
def build_clusters(
    ...
    (
        ...
        _manual_box_store,
        _label_bucket_reject_store,
        _bucket_override_store,
    ) = _setup_runtime(
        min_confidence=min_confidence,
        min_similarity=0.40,
        sources=source,
...
def audit_coverage(
    ...
    """Generate a report of face detection coverage per bucket."""
    log_mod.setup_logging(log_level)
    logger = logging.getLogger("cli.faces_queue.audit")
    cfg = config_mod.load_config()
    conn = db_mod.connect(cfg.db_path)
    
    try:
        report_path = scan_audit.audit_face_coverage(conn, cfg, logger=logger)
...
def main(
    ...
    server = _start_server(
        ...
        label_bucket_reject_store=label_bucket_reject_store,
        bucket_override_store=bucket_override_store,
        sources=source,
        logger=logger,
    )
...
class QueueState:
    def __init__(
        ...
        *,
        ...
        bucket_reject_store: Optional[LabelBucketRejectStore] = None,
        bucket_override_store: Optional[BucketOverrideStore] = None,
    ) -> None:
        self.matcher = matcher
        self.tag_store = tag_store
        self.vote_store = vote_store
        self.ignore_store = ignore_store
        self.bucket_reject_store = bucket_reject_store
        self.bucket_override_store = bucket_override_store
    ...
    def face_ids_for_buckets(self, bucket_prefixes: Iterable[str]) -> Set[str]:
        bucket_set = {prefix.strip() for prefix in bucket_prefixes if isinstance(prefix, str) and prefix.strip()}
        if not bucket_set:
            return set()
        return {record.face_id for record in self.matcher.records if record.bucket_prefix in bucket_set}

    def rejected_bucket_face_ids(self, label: str) -> Set[str]:
        if not self.bucket_reject_store:
            return set()
        buckets = self.bucket_reject_store.rejected_buckets_for(label)
        if not buckets:
            return set()
        return self.face_ids_for_buckets(buckets)

    def effective_min_confidence(self, bucket_prefix: str, default_conf: float) -> float:
        if not self.bucket_override_store:
            return default_conf
        return self.bucket_override_store.get_min_confidence(bucket_prefix, default_conf)

    def all_confidence_overrides(self) -> Dict[str, float]:
        if not self.bucket_override_store:
            return {}
        # We only care about the min_confidence key in each bucket's override data
        res = {}
        for bucket, data in self.bucket_override_store._data.items():
            if "min_confidence" in data:
                res[bucket] = float(data["min_confidence"])
        return res
    ...
    def next_unlabeled(self) -> Optional[FaceRecord]:
        while self._unlabeled_index < len(self.unlabeled_ids):
            ...
            if record is None:
                continue
            # Respect per-bucket override even in seed mode
            if record.confidence < self.effective_min_confidence(record.bucket_prefix, self.min_confidence):
                continue
            return record
        return None
    ...
    def unlabeled_records(self, limit: int, min_confidence: float) -> List[FaceRecord]:
        ...
        for face_id in self.unlabeled_ids:
            ...
            if record is None:
                continue
            if record.confidence < self.effective_min_confidence(record.bucket_prefix, min_confidence):
                continue
    ...
    def _pending_counts(self, min_similarity: float) -> Dict[str, int]:
        ...
        for label, faces in self.label_faces.items():
            ...
            bucket_rejected = self.rejected_bucket_face_ids(label)
            excluded = set(self.labeled_ids) | set(self.ignored_ids) | set(skipped) | set(bucket_rejected)
...
def _start_server(
    ...
    *,
    ...
    label_bucket_reject_store: LabelBucketRejectStore,
    bucket_override_store: BucketOverrideStore,
    sources: Optional[List[str]],
    logger: logging.Logger,
) -> ThreadingHTTPServer:
...
class FaceQueueRequestHandler(SimpleHTTPRequestHandler):
    def __init__(
        ...
        **kwargs,
        ...
        self.label_bucket_reject_store = label_bucket_reject_store
        self.bucket_override_store = bucket_override_store
        self.cfg = cfg
    ...
    def do_GET(self) -> None:  # pragma: no cover - manual
        ...
        if parsed.path == "/api/label/photos":
            ...
            return
        if parsed.path == "/api/label/reject_buckets":
            self._serve_label_reject_buckets(parsed)
            return
    ...
    def do_POST(self) -> None:  # pragma: no cover - manual
        ...
        if endpoint == "/api/photo/redetect":
            ...
            return
        if endpoint == "/api/photo/override":
            self._handle_photo_override()
            return
        if endpoint.startswith("/api/cluster/") and endpoint.endswith("/label"):
            self._handle_cluster_label(endpoint)
            return
        if endpoint == "/api/label/reject_bucket":
            self._handle_label_reject_bucket()
            return
        if endpoint == "/api/label/unreject_bucket":
            self._handle_label_unreject_bucket()
            return
    ...
    def _rejected_buckets_for_label(self, label: str) -> Set[str]:
        if not label or not self.label_bucket_reject_store:
            return set()
        return self.label_bucket_reject_store.rejected_buckets_for(label)

    def _bucket_rejected_face_ids(self, label: str) -> Set[str]:
        buckets = self._rejected_buckets_for_label(label)
        if not buckets:
            return set()
        return self.state.face_ids_for_buckets(buckets)
    ...
    def _serve_clusters(self, parsed) -> None:
        ...
        if offset < 0:
            offset = 0
        try:
            min_faces = int((params.get("min_faces") or [str(CLUSTER_DEFAULT_MIN_FACES)])[0])
        except ValueError:
            min_faces = CLUSTER_DEFAULT_MIN_FACES
        
        try:
            similarity = float((params.get("similarity") or [str(CLUSTER_DEFAULT_SIMILARITY)])[0])
        except ValueError:
            similarity = CLUSTER_DEFAULT_SIMILARITY

        # Trigger rebuild if parameters changed significantly
        if self.cluster_store:
            meta = self.cluster_store.metadata()
            cached_params = meta.get("params") or {}
            cached_min = cached_params.get("min_faces")
            cached_sim = cached_params.get("similarity")
            
            if cached_min != min_faces or (cached_sim is not None and abs(cached_sim - similarity) > 0.001):
                self.logger.info("Rebuilding clusters with new params: min_faces=%s, similarity=%.2f", min_faces, similarity)
                self.cluster_store = _build_and_store_clusters(
                    cfg=self.cfg,
                    matcher=self.state.matcher,
                    state=self.state,
                    photo_status_store=self.photo_status_store,
                    logger=self.logger,
                    min_faces=min_faces,
                    similarity=similarity,
                    force=True
        ...
        for entry in rows:
            ...
            if rep and not rep.get("image_url") and rep.get("image"):
                rep["image_url"] = rep["image"]
    ...
    def _handle_photo_override(self) -> None:
        payload = self._read_payload()
        bucket_prefix = (payload.get("bucket_prefix") or "").strip()
        if not bucket_prefix:
            self.send_error(400, "bucket_prefix is required")
        ...
        if "min_confidence" in payload:
            try:
                val = float(payload["min_confidence"])
                self.bucket_override_store.set_min_confidence(bucket_prefix, val)
            except (ValueError, TypeError):
                self.send_error(400, "min_confidence must be a number")
                return
        
        self._write_json({"status": "ok", "bucket_prefix": bucket_prefix, "overrides": self.bucket_override_store.get(bucket_prefix)})
    ...
    def _photo_faces_payload(self, bucket_prefix: str, variant: Optional[str]) -> Optional[Dict[str, object]]:
        ...
        # Use per-bucket confidence override if present
        effective_min = self.state.effective_min_confidence(bucket_prefix, 0.0)
        
        faces = []
        for record in records:
            if record.confidence < effective_min:
                continue
    ...
    def _assignment_for_face(self, face_id: str, *, tags, votes, ignores) -> Dict[str, Optional[object]]:
        ...
        verdict = None
        if label:
            vote_key = (face_id, normalize_label(label))
    ...
    def _serve_label_photos(self, parsed) -> None:
        ...
        if offset < 0:
            offset = 0
        include_done = self._coerce_bool((params.get("include_done") or ["0"])[0], default=False)
        include_rejected = self._coerce_bool((params.get("include_rejected") or ["0"])[0], default=False)
        try:
            min_confidence = float((params.get("min_confidence") or [str(self.state.min_confidence)])[0])
        except ValueError:
            min_confidence = self.state.min_confidence
        ...
        rejected_meta: Dict[str, Dict[str, str]] = {}
        if self.label_bucket_reject_store:
            rows = self.label_bucket_reject_store.list_for(label)
            for row in rows:
                bucket = (row.get("bucket_prefix") or "").strip()
                if not bucket:
                    continue
                rejected_meta[bucket] = row
        bucket_candidates = set(label_buckets)
        if include_rejected and rejected_meta:
            bucket_candidates |= set(rejected_meta.keys())
        ...
        for record in self.state.matcher.records:
            bucket_prefix = record.bucket_prefix
            if bucket_prefix not in bucket_candidates:
                continue
            ...
            if record.confidence > (entry.get("max_confidence") or 0.0):
                entry["max_confidence"] = record.confidence
        rows = []
        for bucket_prefix in bucket_candidates:
            base = groups.get(bucket_prefix) or bucket_meta.get(bucket_prefix)
            if not base:
                continue
            is_rejected = bucket_prefix in rejected_meta
            ...
            if back_url:
                entry["back_url"] = back_url
            entry["rejected_for_label"] = is_rejected
            if is_rejected:
                entry["rejected_at"] = rejected_meta[bucket_prefix].get("updated_at_utc", "")
            rows.append(entry)
    ...
    def _serve_next_candidate(self, parsed) -> None:
        ...
        bucket_rejected_ids = self._bucket_rejected_face_ids(label)
        excluded = (
            set(self.state.labeled_ids)
            | set(self.state.ignored_ids)
            | set(skipped)
            | self._done_face_ids()
            | set(bucket_rejected_ids)
        )
        candidate = self.state.matcher.next_candidate(
            ...
            min_similarity=min_similarity,
            min_confidence=self.state.min_confidence,
            bucket_conf_overrides=self.state.all_confidence_overrides(),
        )
        if not candidate:
            self._write_json({"status": "empty"})
            return
    ...
    def _handle_label_reject_bucket(self) -> None:
        if not self.label_bucket_reject_store:
            self.send_error(503, "label reject store unavailable")
            return
        payload = self._read_payload()
        label = (payload.get("label") or "").strip()
        bucket_prefix = (payload.get("bucket_prefix") or "").strip()
        if not label or not bucket_prefix:
            self.send_error(400, "label and bucket_prefix are required")
            return
        try:
            record = self.label_bucket_reject_store.add(label, bucket_prefix)
        except ValueError as exc:
            self.send_error(400, str(exc))
            return
        self._write_json(
    ...
    def _handle_label_unreject_bucket(self) -> None:
        if not self.label_bucket_reject_store:
            self.send_error(503, "label reject store unavailable")
            return
        payload = self._read_payload()
        ...
        if not label or not bucket_prefix:
            self.send_error(400, "label and bucket_prefix are required")
            return
        removed = self.label_bucket_reject_store.remove(label, bucket_prefix)
        if not removed:
            self.send_error(404, "bucket was not rejected for that label")
            return
        self._write_json({"status": "ok", "label": label, "bucket_prefix": bucket_prefix, "removed": True})

    def _serve_label_reject_buckets(self, parsed) -> None:
        if not self.label_bucket_reject_store:
            self.send_error(503, "label reject store unavailable")
            return
        params = parse_qs(parsed.query)
        ...
        if label:
            ...
            rows = self.label_bucket_reject_store.all()
        self._write_json({"status": "ok", "label": label or None, "rejects": rows})
