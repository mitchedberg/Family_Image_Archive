...
RECORDER_STATUS_FILE = VOICE_STATE_DIR / "recorder_status.json"
...
def _build_dataset(
    ...
    *,
    ...
    voice_root = cfg.staging_root / "02_WORKING_BUCKETS" / "voice_sessions" / "transcripts"
...
class ReviewRequestHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # pragma: no cover - exercised manually
        ...
        if parsed.path == "/api/photo/transform":
            ...
            return
        if parsed.path == "/api/recorder_status":
            self._serve_recorder_status()
            return
        if parsed.path == "/api/voice_data":
            self._serve_voice_data(parsed)
            return
        super().do_GET()

    def _serve_voice_data(self, parsed) -> None:
        ...
        # We need the path to VOICE_SESSIONS_ROOT/transcripts
        # In _build_dataset:
        # voice_root = cfg.staging_root / "02_WORKING_BUCKETS" / "voice_sessions" / "transcripts"
        # Since we don't have direct access to 'cfg' in the handler instance,
        # we can reconstruct it from self.directory (which is cfg.staging_root / "02_WORKING_BUCKETS")
        base_dir = Path(self.directory)
        voice_root = base_dir / "voice_sessions" / "transcripts"
        
        disk_voice = _load_voice_transcripts_from_disk(voice_root, bucket_prefix)
        
        # We also need to fetch existing embedded transcripts from the bucket's sidecar
        # to properly merge them, similar to _build_dataset.
        bucket_dir = base_dir / "buckets" / f"bkt_{bucket_prefix}"
        ...
        if sidecar_path.exists():
            try:
                ...
                voice_transcripts = bucket_data.get("voice_transcripts")
                if isinstance(voice_transcripts, list):
                    for record in voice_transcripts:
                        if not isinstance(record, dict):
                            continue
    ...
    def _serve_recorder_status(self) -> None:
        if not RECORDER_STATUS_FILE.exists():
            self._write_json({"recording": False})
            return
        try:
            payload = json.loads(RECORDER_STATUS_FILE.read_text(encoding="utf-8"))
            if not isinstance(payload, dict):
                raise ValueError("Invalid recorder status payload")
        except Exception:
